defmodule CapybaraWeb.CounterLive.FormComponent do
  use CapybaraWeb, :live_component
  ...
  def update(%{counter: counter} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Example.change_counter(counter))
     end)}
  end

  @impl true
  def handle_event("validate", %{"counter" => counter_params}, socket) do
    changeset = Example.change_counter(socket.assigns.counter, counter_params)
  ...
  defp save_counter(socket, :edit, counter_params) do
    case Example.update_counter(socket.assigns.counter, counter_params) do
      {:ok, counter} ->
  ...
  defp save_counter(socket, :new, counter_params) do
    case Example.create_counter(counter_params) do
      {:ok, counter} ->
        notify_parent({:saved, counter})
